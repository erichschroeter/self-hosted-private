...
- **Environment-Driven Configuration**: Parameterizing deployment files using environment files (`.env`). Specific context environments (such as home and work) are stored in `env/home.env` and `env/work.env` respectively.
  - *Constraint*: Local secrets or environment-specific values must never be hardcoded into the `compose.yaml` file; they must be referenced via environment variables and loaded from a `.env` file dynamically linked and managed by the `switch-context.sh` framework.
...
- **Context-Aware Symlinking (`switch-context.sh`)**: A centralized, zero-hardcode automation framework that traverses all services in both public and private repositories, dynamically creating symbolic links for `.env` and `*.<context>.<ext>` custom config files (such as Homepage's services or Arcane's overrides).
- **Secrets Isolation (`secrets.env`)**: A secure design pattern where sensitive credentials (API keys, passwords, database root keys) are isolated into a local, uncommitted `secrets.env` file. This file is merged into the container's environment using Docker Compose's service-level `env_file` block, leaving the main `home.env` / `work.env` templates clean and leak-proof.
